...
if __name__ == '__main__':
    ...
    l = link.Link(address=2,startpoint=n2,endpoint=n1,bandwidth=1000000.0,propagation=0.1)
    n2.add_link(l)
    n2.add_forwarding_entry(address=1,link=l)

    # create link from 2 to 3
    l = link.Link(address=2,startpoint=n2,endpoint=n3,bandwidth=1000000.0,propagation=0.1)
    n2.add_link(l)
    n2.add_forwarding_entry(address=3,link=l)

    # create link from 3 to 2
    l = link.Link(address=3,startpoint=n3,endpoint=n2,bandwidth=1000000.0,propagation=0.1)
    n3.add_link(l)
    n3.add_forwarding_entry(address=2,link=l)
    n3.add_forwarding_entry(address=1,link=l)
    
    # create Delay Handler at n3 to print results
    d = DelayHandler()
